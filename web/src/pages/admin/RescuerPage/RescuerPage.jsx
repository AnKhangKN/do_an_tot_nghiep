...
const RescuerPage = () => {
  ...
  const fetchAnalytics = async () => {
    try {
      setLoadingPerf(true);
      const res = await getRescuerAnalytics(perfPage, 10, search);
      if (res?.success) {
        setData(res.data?.data || []);
        setPerfTotalPages(res.data?.totalPages || 1);
  ...
  const tabClass = (isActive) =>
    `px-3 py-1.5 text-xs font-semibold rounded-xl transition-all cursor-pointer ${isActive
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-2 border-b border-gray-100">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Quản lý người cứu hộ</h1>
          <p className="text-sm text-gray-500 mt-0.5">Duyệt hồ sơ và theo dõi hiệu suất của đội ngũ cứu hộ viên</p>
        </div>
        ...
        <div className="flex bg-gray-100 p-1 rounded-2xl border border-gray-200">
          ...
          <button
            ...
            className={tabClass(activeTab === 'performance')}
          >
            Hiệu suất Cứu hộ
          </button>
        </div>
