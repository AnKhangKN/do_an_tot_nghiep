  ...
  return (
    <div>
      ...
      <AddUpdateModelComponent
        ...
        title={actionModal?.action === 'approve' ? 'Duyệt đơn kháng cáo' : 'Từ chối đơn kháng cáo'}
        subtitle={actionModal ? `${actionModal.user_name} (${actionModal.user_email})` : undefined}
        headerIcon={
          <div
            className={`p-2 rounded-2xl ${
              actionModal?.action === 'approve' ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600'
            }`}
          >
            {actionModal?.action === 'approve' ? <PiCheck size={24} /> : <PiX size={24} />}
          </div>
        }
        onSubmit={() => handleAction(actionModal?.action === 'approve')}
        submitLabel={actionModal?.action === 'approve' ? 'Xác nhận duyệt' : 'Xác nhận từ chối'}
        submitVariant={actionModal?.action === 'approve' ? 'success' : 'danger'}
