      ...
      render: (row) => (
        <div className="max-w-[240px]">
          <p className="text-xs text-gray-800 font-medium line-clamp-2 leading-relaxed">
            {row.textContent || <span className="text-gray-400 italic">Không có mô tả văn bản</span>}
          </p>
        </div>
      )
    },
        ...
        <div>
          {row.isFlagged ? (
            <div className="flex items-center gap-1.5 text-rose-600 bg-rose-50 px-3 py-1 rounded-xl border border-rose-200">
              ...
              <span className="text-xs font-semibold">Spam - Bất thường</span>
            </div>
          ) : (
          ...
          {row.flagReason && (
            <p className="text-[11px] text-rose-500 mt-1 italic leading-tight max-w-[200px]">
              "{row.flagReason}"
            </p>
          )}
        </div>
          ...
          <div className="flex flex-col gap-1 items-start">
            ...
            {row.reviewerName && (
              <span className="text-[11px] text-gray-500 font-medium flex items-center gap-1">
                <PiUserFill size={12} className="text-gray-400" />
                {row.reviewerName}
              </span>
            )}
          </div>
        ...
        return (
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-1.5 rounded-xl bg-gray-100 hover:bg-gray-200 text-gray-700 transition-all cursor-pointer border border-gray-200 active:scale-95"
              title="Xem chi tiết"
            >
              <PiEyeFill size={16} />
            </button>
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-rose-900 hover:bg-rose-800 text-white text-xs font-medium transition-all disabled:opacity-40 cursor-pointer active:scale-95 shadow-xs"
              title="Xác nhận nội dung vi phạm/spam"
            >
              Vi phạm
            </button>
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-emerald-50 hover:bg-emerald-100 text-emerald-700 text-xs font-medium transition-all disabled:opacity-40 cursor-pointer active:scale-95 border border-emerald-200"
              title="Xác nhận nội dung an toàn/hợp lệ"
            >
              An toàn
            </button>
          </div>
            ...
            <div className="space-y-4 text-xs">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-gray-50 p-3.5 rounded-2xl border border-gray-100 space-y-1">
                  <span className="text-gray-400 font-medium block">Chủ đề AI gợi ý</span>
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold border ${categoryBadgeStyles[selectedLog.suggestedCategory] || categoryBadgeStyles['KHÁC']}`}>
                    {selectedLog.suggestedCategory || 'KHÁC'}
                  </span>
                  <p className="text-[11px] text-gray-500 mt-1">
                    Độ tin cậy: <span className="font-bold text-gray-800">{Math.round((selectedLog.aiScore || 0) * 100)}%</span>
                  </p>
                </div>
