...
import { appealBanPublic } from "@/api/shared/AuthApi";
...
function App() {
  ...
  const handleAppealSubmit = async () => {
    ...
    setAppealError("");
    try {
      await appealBanPublic(appealEmail.trim(), appealText.trim());
      setAppealSent(true);
      setShowAppealForm(false);
    } catch (error) {
      const msg = error?.response?.data?.message || "Không thể gửi yêu cầu kháng cáo!";
      if (error?.response?.status === 401 || error?.response?.status === 404) {
        setAppealError("Email không tồn tại trong hệ thống!");
      } else {
        setAppealError(msg);
      }
    } finally {
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 px-5 py-2.5 rounded-2xl bg-white text-gray-700 hover:bg-gray-100 text-xs font-bold transition border border-gray-200 cursor-pointer"
                >
                  Quay lại
                </button>
                <button
                  onClick={handleAppealSubmit}
                  disabled={!appealEmail.trim() || !appealText.trim() || appealSending}
                  className="flex-1 px-5 py-2.5 rounded-2xl bg-gray-900 text-white hover:bg-gray-800 text-xs font-bold transition shadow-md disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                >
          ...
          <div className="bg-white rounded-3xl max-w-md w-full mx-4 shadow-2xl overflow-hidden">
            <div className="p-8 text-center">
              ...
              <button
                ...
                className="w-full px-5 py-3 rounded-2xl bg-gray-900 text-white hover:bg-gray-800 text-sm font-bold transition shadow-md cursor-pointer"
              >
                Đã hiểu
              </button>
            </div>
