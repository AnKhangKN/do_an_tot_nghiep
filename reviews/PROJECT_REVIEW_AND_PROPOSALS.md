...
1. **Quên mật khẩu & Đặt lại mật khẩu (Forgot & Reset Password via OTP/Email)**: Bổ sung luồng khôi phục mật khẩu tài khoản cho người dùng và cứu hộ viên qua mã OTP gửi về Email hoặc liên kết reset bảo mật, giúp tự khôi phục quyền truy cập tài khoản an toàn khi lỡ quên mật khẩu đăng nhập.
2. **Quản trị khóa tài khoản vi phạm (Account Suspension & Ban Management)**: Cung cấp công cụ cho phép Admin thực hiện khóa tạm thời hoặc vĩnh viễn tài khoản của Nạn nhân (Victim) hoặc Cứu hộ viên (Rescuer) khi phát hiện hành vi vi phạm tiêu chuẩn cộng đồng (như tạo SOS giả, báo cáo thông tin sai sự thật hoặc vi phạm ứng xử), đồng thời tự động thu hồi Token và vô hiệu hóa quyền truy cập hệ thống.
3. **Tự động đóng kênh nhắn tin khi hoàn thành ca cứu hộ (Post-Rescue Chat Auto-Closure)**: Ngay khi ca cứu hộ chuyển sang trạng thái "Hoàn thành" (hoặc "Đã hủy"), hệ thống tự động khóa luồng trò chuyện giữa Nạn nhân (Victim) và Cứu hộ viên (Rescuer), ngăn hai bên tiếp tục gửi tin nhắn cho nhau nhằm tránh tình trạng nhắn các nội dung không phù hợp hoặc gây phiền phức sau ca cứu hộ, giúp xây dựng cộng đồng văn minh và an toàn hơn.
4. **Tự động hủy yêu cầu cứu hộ khẩn cấp (Automatic Emergency Rescue Cancellation)**: Khi Nạn nhân (Victim) không gửi yêu cầu cứu hộ khẩn cấp (SOS) sau một khoảng thời gian nhất định kể từ lúc bật ứng dụng và ở trạng thái không hoạt động (ví dụ: 15 phút hoặc 30 phút không tương tác), hệ thống tự động coi như Nạn nhân không còn cần trợ giúp khẩn cấp và tự động hủy yêu cầu cứu hộ, từ chối giao tiếp với Cứu hộ viên, đồng thời hiển thị thông báo lý do hủy cho Nạn nhân để tránh lãng phí thời gian và nguồn lực của Cứu hộ viên.
5. **Giao diện quản lý thông tin chuyên biệt theo từng loại hình & cấp độ (Role-Based Information Management & Hierarchy)**: Xây dựng các trang quản lý chuyên biệt, có thể điều chỉnh linh hoạt theo các cấp độ quản trị khác nhau (ví dụ: Cấp Quốc gia, Tỉnh/Thành phố, Quận/Huyện, hoặc Cấp Tổ chức/Đội cứu hộ), cho phép quản lý riêng biệt về Cấu hình Hệ thống, Báo cáo Vận hành, Quản lý Tiện ích, Quản lý Người dùng, và Quản lý Điểm Cảnh báo tùy theo từng cấp quản trị, giúp Admin dễ dàng kiểm soát và vận hành hệ thống theo đúng vai trò và phạm vi trách nhiệm của mình.
6. **Tự động gộp và loại bỏ các tiện ích trùng lặp và vi phạm (Automatic Emergency Amenities Duplication & Violation Removal)**: Phát triển hệ thống tự động kiểm tra các tiện ích khẩn cấp (Emergency Amenities) được người dùng tạo mới hoặc nhập thủ công, thực hiện tự động gộp các tiện ích có vị trí trùng lặp hoặc rất gần nhau vào một (Merge), đồng thời tự động xóa bỏ các tiện ích vi phạm các quy định đã được cấu hình (như vi phạm về khoảng cách tối thiểu, hình ảnh không phù hợp, hoặc không thuộc loại hình tiện ích được phép), nhằm giữ gìn sự gọn gàng và chính xác cho bản đồ tiện ích.
7. **Tự động gộp các điểm cảnh báo rủi ro trùng lặp (Automatic Hazard Point Clustering & Merging)**: Phát triển hệ thống tự động quét và phát hiện các điểm cảnh báo rủi ro có vị trí trùng lặp hoặc rất gần nhau trên bản đồ, sau đó tự động gộp tất cả các điểm này thành một điểm cảnh báo duy nhất (Cluster & Merge) và tổng hợp thông tin từ các báo cáo gốc vào một điểm cảnh báo hợp nhất, đảm bảo thông tin cảnh báo được hiển thị một cách chính xác, khoa học và tránh tình trạng phân tán nhiều điểm trùng lặp trên bản đồ.
8. **Tích hợp API Google Translate và AI Tóm tắt Google Gemini cho Kênh Hỗ trợ Khẩn cấp (Google Translate & Gemini AI Summarization Integration for Emergency Support)**: Tích hợp sâu API Google Translate và AI Tóm tắt Gemini để dịch tự động các nội dung quan trọng giữa các ngôn ngữ khác nhau trong kênh hỗ trợ khẩn cấp, đồng thời sử dụng AI Gemini để tóm tắt các trao đổi phức tạp thành bản tóm tắt ngắn gọn, giúp Nạn nhân, Cứu hộ viên và Admin hiểu nhanh tình hình mà không cần đọc toàn bộ nội dung dài dòng.
9. **Tự động tính toán trọng số độ nguy cấp và cập nhật Heatmap thời gian thực (Real-time Hazard Heatmap with Dynamic Criticality Weight Calculation)**: Xây dựng thuật toán tự động tính toán trọng số độ nguy cấp của từng điểm cảnh báo/vùng nguy hiểm dựa trên các yếu tố như: loại hình sự cố, thời gian phát hiện, mức độ hoạt động của cộng đồng xung quanh, phản hồi xác thực từ người dùng, và mức độ ưu tiên do Admin thiết lập; sau đó tự động cập nhật lại Heatmap thời gian thực để hiển thị chính xác các vùng có nguy cơ cao nhất cho Cứu hộ viên và Nạn nhân.
10. **Tự động kiểm tra và tính điểm xếp hạng chất lượng cứu hộ (Automated Rescue Quality Rating & Scoring)**: Tự động tính toán điểm xếp hạng chất lượng cứu hộ (Rescuer Rating) dựa trên các yếu tố định lượng và khách quan như: thời gian phản hồi, thời gian đến hiện trường, thời gian hoàn thành ca cứu hộ, và đánh giá trung bình từ Nạn nhân (Avg Rating); đồng thời tự động cập nhật điểm xếp hạng này lên hồ sơ Cứu hộ viên, giúp Nạn nhân và cộng đồng dễ dàng lựa chọn Cứu hộ viên uy tín và tin cậy.
